...
TELEGRAM_BOT_TOKEN="YOURTELEBOTTOKEN"
TELEGRAM_CHAT_ID="YOURTELEGRAMCHATID"

#File containing the list of DNS servers
DNS_SERVERS_FILE="PATH/dns_servers.txt"

# Output files
FUNCTIONABLE_FILE="PATH/$DOMAINNAME/${DOMAINNAME}functionabledns.txt"
NONFUNCTIONABLE_FILE="PATH/$DOMAINNAME/${DOMAINNAME}nonfunctionabledns.txt"
PREVIOUS_NONFUNCTIONABLE_FILE="PATH/$DOMAINNAME/previous_${DOMAINNAME}nonfunctionabledns.txt"
...
check_dns () {
	...
	if [[ "$cname" == "$EXPECTED_CNAME" ]]; then
		...
		# Check if the CNAME contains the keyword ".id."
		if [[ "$cname" == *".id." ]]; then
...
if [[ -f "$PREVIOUS_NONFUNCTIONABLE_FILE" ]]; then
	...
	if (( current_nonfunctionable_count > previous_nonfunctionable_count )); then
		alert_message="IDDNS Alert for ${DOMAIN}: The number of non-functionable DNS servers has increased! The domain ${DOMAIN} might be blocked.\n$alert_message"
	...
	alert_message="IDDNS for ${DOMAIN}: Current non-functionable count: $current_nonfunctionable_count out of $total_dns_servers\n\
...
if [[ -s "$BLOCKED_DNS_FILE" ]]; then
	...
	blocked_alert_message="IDDNS Alert for ${DOMAIN}: There are $blocked_count blocked DNS servers for the domain ${DOMAIN} listed in gwmylinkportalnetblockedDNS.txt."
